...
//     setIsJoining(true);
//     try {
//       // Get the userId from the stored profile, fallback to 1 as requested in payloads
//       const profileStr = localStorage.getItem("nea_user_profile");
//       const userId = profileStr ? JSON.parse(profileStr).id : 1;
      
//       // Perform the join actions in parallel
//       await Promise.all([
//         apiService.joinEvent(event.eventId, userId),
//         apiService.updateUserJoinedEvents(userId, event.eventId),
//       ]);
      
//       // Sync local UI Profile without needing a hard reload
//       if (profileStr) {
...
//       // Even on error, we might fall back to success for mock purposes, but error toast could be added here
//     } finally {
//       setIsJoining(false);
//     }
//   };
...
//               disabled={isJoining}
//               className="flex-1 py-3 bg-[#428b5e] hover:bg-[#34704b] text-white font-bold rounded-lg transition-colors cursor-pointer disabled:opacity-50 flex items-center justify-center gap-2"
//             >
//               {isJoining ? (
//                 <>
...
//       <div className="bg-white md:rounded-[2rem] w-full max-w-2xl max-h-screen md:max-h-[85vh] flex flex-col relative overflow-hidden shadow-2xl">
...
//                     <p className="font-semibold text-gray-900 text-sm">{event.joinsCount?.toLocaleString() || 0}</p>
//                   </div>
//                 </div>
...
//                   <p className="text-gray-700 text-sm font-medium">{event.details || "No details provided."}</p>
//                 </div>
//               </div>
...
//                 <p className="text-gray-600 leading-relaxed text-sm">
//                   {event.rewards || "No rewards specified."}
//                 </p>
//               </div>
//             </div>
//         </div>
...
//         <div className="p-4 bg-white border-t border-gray-100 shrink-0 shadow-[-10px_-10px_30px_0_rgba(0,0,0,0.05)]">
//           <button 
