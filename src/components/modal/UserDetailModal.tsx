...
function getBadgeInfo(totalHours: number) {
  if (totalHours >= 150)
    return { current: "Diamond", next: null, progress: 100, color: "#509CD1" };
  if (totalHours >= 100)
    return {
      current: "Gold",
      next: "Diamond",
      progress: Math.round(((totalHours - 100) / 50) * 100),
      color: "#86B537",
    };
  if (totalHours >= 50)
    return {
      current: "Silver",
      next: "Gold",
      progress: Math.round(((totalHours - 50) / 50) * 100),
      color: "#A0AAB5",
    };
  return {
    current: "Bronze",
    ...
    >
      ...
      >
        ...
        <div
          ...
          style={{
            background: "linear-gradient(135deg, #0F2D3C 0%, #1A4A5F 100%)",
          }}
        >
          <button
            onClick={onClose}
        ...
        <div className="pt-10 px-6 pb-6">
          ...
          <div className="space-y-3 mb-5">
            <div className="flex items-center gap-3 text-sm">
              ...
              <div>
                <p className="text-[10px] font-semibold uppercase tracking-wider text-[#8A9AA8]">
                  Email
                </p>
            ...
            <div className="grid grid-cols-3 gap-3">
              ...
              <div className="flex items-center gap-2 text-sm">
                ...
                <div>
                  ...
                  <p className="font-semibold text-[#1A2A3A] capitalize">
                    {user.gender ?? "—"}
                  </p>
                </div>
              </div>
          ...
          >
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-semibold text-[#1A2A3A]">
                  {badgeInfo.current} Badge
                </span>
              </div>
              {badgeInfo.next && (
                <span className="text-xs text-[#8A9AA8] font-medium">
                  Next:{" "}
                  <span className="font-semibold text-[#6B7A88]">
                    {badgeInfo.next}
                  </span>
                </span>
              )}
            ...
            <p className="text-[11px] text-[#8A9AA8] font-medium mt-1.5">
              ...
              {badgeInfo.next &&
                ` · ${(Math.ceil(totalHours / 50) * 50 - totalHours).toFixed(1)}h to ${badgeInfo.next}`}
            </p>
          </div>
              ...
              <div className="max-h-32 overflow-y-auto space-y-1">
                {user.joinedEvents.map((evId) => (
                  ...
                  >
                    ...
                    />
                    <span className="text-xs font-medium text-[#6B7A88] font-mono">
                      {evId}
                    </span>
                  </div>
                ))}
