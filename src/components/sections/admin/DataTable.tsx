...
export function DataTable<T extends Record<string, unknown>>({
  ...
  const SortIcon = ({ colKey }: { colKey: string }) => {
    if (sortKey !== colKey)
      return <ChevronsUpDown size={13} className="text-[#C5CDD5]" />;
    if (sortDir === "asc")
      return <ChevronUp size={13} style={{ color: "#108ACB" }} />;
              ...
              {columns.map((col) => (
                <th
                  ...
                  className={`px-4 py-3 text-left text-[10px] font-semibold uppercase tracking-wider text-[#8A9AA8] whitespace-nowrap ${
                    col.sortable
                      ? "cursor-pointer select-none hover:text-[#6B7A88]"
                      : ""
                  }`}
            ...
            {sortedData.length === 0 ? (
              <tr>
                <td colSpan={columns.length} className="py-16 text-center">
                  ...
                  <p className="text-[#1A2A3A] font-semibold text-sm">
                    {emptyText}
                  </p>
                    ...
                    <td key={col.key} className="px-4 py-3 align-middle">
                      {col.render
                        ...
                        : ((row[col.key] as React.ReactNode) ?? (
                            <span className="text-[#C5CDD5]">—</span>
                          ))}
                    </td>
                  ))}
                </tr>
